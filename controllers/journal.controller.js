...
const createJournal = async (req, res) => {
  try {
    ...
    const [createJournal] = await sequelize.query(
      'INSERT INTO `journal` (`name`, `category`, `eissn`, `pissn`, `sjif`, `isi`, `status`, `short_code`, `journal_no`) VALUES (:name, :category, :eissn, :pissn, :sjif, :isi, :status, :shortCode, :journal_no)',
      {
...
const updateJournal = async (req, res) => {
  try {
    ...
    const [updateJournal] = await sequelize.query(
      'UPDATE `journal` SET `name` = :name, `category` = :category, `eissn` = :eissn, `pissn` = :pissn, `sjif` = :sjif, `isi` = :isi, `status` = :status, `short_code` = :shortCode, `updated_at`= :updatedAt, `journal_no` =:journal_no WHERE `id` = :id',
      {
        replacements: req.body,
      }
    );
